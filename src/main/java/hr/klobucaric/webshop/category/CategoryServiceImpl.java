	...
	public CategoryDto save(CategoryCommand command) {
		try {
			...
			if(categoryParentIdIsNull) {
				...
				Category parentCategory = categoryRepository.findById(command.getParentCategoryId()).orElseThrow(
						() -> new ApiBadRequestException("There is no parent category with id: "+ command.getParentCategoryId())
				);
