...
public class ShoppingCartItemServiceImpl implements ShoppingCartItemService {
    ...
    public ShoppingCartItemDto addToCart(ShoppingCartItemCommand command) {
        return mapShoppingCartToDto(shoppingCartItemRepository
                .save(mapCommandToShoppingCartItem(command)));

    }

    private ShoppingCartItemDto mapShoppingCartToDto(ShoppingCartItem cartItem) {
        return new ShoppingCartItemDto(cartItem.getId(),cartItem.getQty(),
                cartItem.getProductItem().getId(),cartItem.getShoppingCart().getId());
    }
    ...
    private ShoppingCartItem mapCommandToShoppingCartItem(ShoppingCartItemCommand command) {
        ...
        shoppingCartItem.setProductItem(productItemRepository.findById(command.getProductItemId()).orElseThrow(
